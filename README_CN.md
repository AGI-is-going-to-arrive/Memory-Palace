...
- **部署更稳了**：Docker 一键脚本补了 deployment lock，运行时环境注入默认关闭，发布前也有自检脚本兜底。
- **高干扰检索更强了**：对照旧版本时，`s8,d200` 与 `s100,d200` 这类更容易被干扰的场景，C/D 档位召回明显提升。
...
> 这里保留**对用户有用的摘要表**。更细的 benchmark 原始日志、一次性重测草稿和机器相关产物默认只保存在本地，不作为公开仓承诺的一部分。
>
> 想看方法、边界和复现命令，直接看 `docs/EVALUATION.md`；想看这次相对旧项目到底升级了什么，直接看 `docs/changelog/release_summary_vs_old_project_2026-03-06.md`。
